...
def get_tracks(track_ids: list[str]) -> pd.DataFrame:
    p = requests.get(
        "https://api.spotify.com/v1/tracks?ids=" + ",".join(track_ids), headers=headers
    )
        ...
        [
            {
                ...
                "genres": [
                    genre for artist in track["artists"] for genre in artist["genres"]
                ],
            }
...
def get_tracks_individual(track_ids: list[str]) -> pd.DataFrame:
    ...
    for track in track_ids:
        p = requests.get("https://api.spotify.com/v1/tracks/" + track, headers=headers)
...
def get_features_individual(track_ids: list[str]) -> pd.DataFrame:
    ...
    for song_id in track_ids:
        p = requests.get(
            "https://api.spotify.com/v1/audio-features/" + song_id, headers=headers
        )
        if p.status_code != 200:
            json = {}
        else:
...
def get_date(album: dict) -> datetime:
    ...
    """
    date_format = {"day": "%Y-%m-%d", "month": "%Y-%m", "year": "%Y"}
...
def get_spotify_data(music_league_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.concat(
        (
            music_league_df.set_index(np.arange(len(music_league_df))),
            track_data,
            feature_data,
        ),
        axis=1,
